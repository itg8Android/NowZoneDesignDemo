package itg8.com.nowzonedesigndemo.breath.timeline;
...
class DynamicSpannableString implements CharSequence, GetChars, Spannable {
    ...
    public CharSequence subSequence(int start, int end) {
        return new DynamicSpannableString(this, start, end);
    }
    ...
    public void setSpan(Object what, int start, int end, int flags) {
        ...
        if (mSpanCount + 1 >= mSpans.length) {
            Object[] newTags = new Object[mSpanCount <= 4 ? 8 : mSpanCount * 2];
    ...
    public void removeSpan(Object what) {
        ...
        for (int i = count - 1; i >= 0; i--) {
            if (spans[i] == what) {
                int start = data[i * COLUMNS + START];
                int end = data[i * COLUMNS + END];
                ...
                sendSpanRemoved(what, start, end);
                return;
            }
        }
    }
    ...
    public <T> T[] getSpans(int queryStart, int queryEnd, Class<T> kind) {
        ...
        for (int i = 0; i < spanCount; i++) {
            ...
            if (count == 0) {
                ...
                int priority = data[i * COLUMNS + FLAGS] & Spanned.SPAN_PRIORITY;
                if (priority != 0) {
                    int j;
    ...
    private void sendSpanAdded(Object what, int start, int end) {
        SpanWatcher[] spans = getSpans(start, end, SpanWatcher.class);
    ...
    private void sendSpanChanged(Object what, int s, int e, int st, int en) {
        SpanWatcher[] spans = getSpans(Math.min(s, st), Math.max(e, en), SpanWatcher.class);
        for (SpanWatcher watcher : spans) {
            watcher.onSpanChanged(this, what, s, e, st, en);
        }
    }
