package itg8.com.nowzonedesigndemo.home.ProgressView;
...
public class CircleProgressView extends View {
    ...
    private float mTextStepScale = 1;
...
//    private int mInnerContourColor = 0xAA000000;
    ...
    // change it now
    private int mOuterContourColor = 0xFFFFFFFF;
    private int mInnerContourColor = 0xFFFFFFFF;
...
//        theta += Math.PI/2.0;
    ...
    private void parseAttributes(TypedArray a) {
        ...
        if (a.hasValue(R.styleable.CircleProgressView_cpv_textMode)) {
            setTextMode(TextMode.values()[a.getInt(R.styleable.CircleProgressView_cpv_textMode, 3)]);
        }
        ...
        // Add it Now...
        if (a.hasValue(R.styleable.CircleProgressView_cpv_text)) {
            setText(a.getString(R.styleable.CircleProgressView_cpv_text_steps));
        }
        ...
        if (a.hasValue(R.styleable.CircleProgressView_cpv_blockCount)) {
            setBlockCount(a.getInt(R.styleable.CircleProgressView_cpv_blockCount, 0));
        ...
        if (mShowUnit) {
            ...
            switch (mUnitPosition) {
                ...
                case BOTTOM:
                    ...
                    textRectStep = textRect;
                   // textRectStep = new RectF(mOuterTextBounds.left, mOuterTextBounds.top, mOuterTextBounds.right, mOuterTextBounds.bottom+10);
                    break;
                case LEFT_TOP:
                case LEFT_BOTTOM:
    ...
    private void drawTextWithUnit(Canvas canvas) {
        ...
        switch (mUnitPosition) {
            case TOP:
                relativeGapWidth = 0.02f; //gap size between text and unit
                relativeGapHeight = 0.015f; //gap size between text and unit
                relativeHeight = 0.15f * mRelativeUniteSize;
                relativeWidth = 0.2f * mRelativeUniteSize;
                break;
        ...
        if (DEBUG) {
           ...
           mStepPaint= mTextPaint;
        mStepPaint.setTextSize(calcTextSizeForRect(stepText, mStepPaint, textRectStep) * mTextScale);
        mActualTextBounds = calcTextBounds(text, mTextPaint, textRectStep);

        canvas.drawText(text, mActualTextBounds.left - (mTextPaint.getTextSize() * 0.02f), mActualTextBounds.bottom, mTextPaint);
        canvas.drawText(stepText, mActualTextBounds.left - (mStepPaint.getTextSize() * 0.02f), mActualTextBounds.bottom, mStepPaint);
